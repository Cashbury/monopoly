class Users::UsersSnapsController < Users::BaseController
	def snap
      ...
      elsif !qr_code.engagement.is_started
        respond_with_error("Engagement no longer running")          
      else
        respond_to do |format|
          account,campaign,program,after_fees_amount=current_user.snapped_qrcode(qr_code,params[:place_id],params[:lat],params[:long])
  ...
  private
  def snap_hash(account,campaign,program,after_fees_amount)
    s = {:snap => {}}
