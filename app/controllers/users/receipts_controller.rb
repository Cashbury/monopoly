class Users::ReceiptsController < Users::BaseController
  def index
    all_receipts=current_user.receipts
                             ...
                             .select("receipts.log_group_id,receipts.amount,receipts.business_id, receipts.place_id, receipts.receipt_text,receipts.receipt_type, receipts.user_id, receipts.transaction_id, receipts.created_at as date_time, places.name as place_name, brands.name as brand_name")
    result={}
    result[:receipts]=[]
    all_receipts.each_with_index do |receipt,index|
      ...
      if log_group.present?
        ...
        logs=log_group.logs.joins(:engagement=>:item).select("logs.gained_amount as amount, engagements.name as title, logs.frequency as quantity")
