...
class LoyalCustomersControllerTest < ActionController::TestCase
  ...
  setup do
    ...
    10.times {@user1.snapped_qrcode(@qr_code,@engagement,@place.id,@place.lat,@place.long)}
    5.times {@user2.snapped_qrcode(@qr_code,@engagement,@place.id,@place.lat,@place.long)}
  end
  test "should get index" do
    ...
    assert_equal assigns(:results)[0].total.to_i,10
    assert_equal assigns(:results)[0].first_name,@user1.first_name
    assert_equal assigns(:results)[0].last_name,@user1.last_name
    assert_equal assigns(:results)[1].total.to_i,5
    assert_equal assigns(:results)[1].first_name,@user2.first_name
    assert_equal assigns(:results)[1].last_name,@user2.last_name
